...
/**
 * Implements soft line wrapping on an appendable. To use, append characters using [.append]
 * or soft-wrapping spaces using [.wrappingSpace].
 */
internal class LineWrapper(private val out: Appendable,
                           private val indent: String, private val columnLimit: Int) {
    ...
    fun append(s: String) {
        ...
        column = when {
            lastNewline != -1 -> s.length - lastNewline - 1
    ...
    private fun flush(wrap: Boolean) {
        if (wrap) {
            out.append('\n')
            (0..indentLevel - 1).forEach { _ -> out.append(indent) }
